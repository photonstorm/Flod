package neoart.flod.digitalmugician {
  ...
  public final class DMPlayer extends AmigaPlayer {
    ...
    override protected function process():void {
      var chan:AmigaChannel, com:DMCommand, dst:int, i:int, idx:int, j:int, len:int, r:int, src1:int, src2:int, sample:DMSample, value:int, tables:Vector.<int>, voice:DMVoice;
      ...
      //for (i = 1; i < 2; ++i) {
        ...
        if (i < 3 || numChannels == 4) {
          chan = amiga.channels[i];
          if (patternEnd) voice.step = song1.tracks[int(stepCnt + i)];
          ...
          chan = mixChannel;
          if (patternEnd) voice.step = song2.tracks[int(stepCnt + (i - 3))];
        }

        if (commandEnd) {
          com = mod.patterns[int(voice.step.pattern + patternCnt)];
...
//printWave(src2, 128);
      ...
      if (numChannels > 4) {
        ...
        for (i = 0; i < 350; ++i) {
          ...
          for (j = 3; j < 7; ++j) {
            voice = voices[j];
            src2 = (tables[int(voice.mixPtr + (voice.mixStep >> 16))] & 255) + voice.mixVolume;
            dst += volumes[src2];
            voice.mixStep += voice.mixSpeed;
          }

          tables[src1++] = averages[dst];
        }
      ...
      if (--timer == 0) {
        ...
        if (patternCnt == 64 || patternCnt == patternLen) {
          ...
          if (stepCnt == song1.length) {
            stepCnt = song1.loopStep;
          }
        }
      } else {
        commandEnd = 0;
      ...
      //for (i = 1; i < 2; ++i) {
        ...
        chan = i > 3 ? mixChannel : amiga.channels[i];
        //if (i == 1) chan.enabled = 1;
        chan.enabled = 1;
      }
    }
...
/*
    private function printWave(index:int, length:int):void {
      var i:int, len:int = index + length, p:int, s:String = "", tables:Vector.<int> = amiga.samples;
