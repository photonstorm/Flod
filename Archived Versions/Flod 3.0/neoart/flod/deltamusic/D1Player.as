...
package neoart.flod.deltamusic {
  ...
  public final class D1Player extends AmigaPlayer {
    ...
    override protected function loader(stream:ByteArray):void {
      var data:Vector.<int>, i:int = 0, id:String, index:int, j:int, len:int, position:int, row:AmigaRow, sample:D1Sample, step:AmigaStep, value:int;
      id = stream.readMultiByte(4, ENCODING);
      if (id != "ALL ") return;

      position = 104;
      data = new Vector.<int>(25 ,true);
      for (; i < 25; ++i) data[i] = stream.readUnsignedInt();
